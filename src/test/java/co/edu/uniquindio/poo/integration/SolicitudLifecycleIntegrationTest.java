...
class SolicitudLifecycleIntegrationTest {
        ...
        @Test
        @Order(2)
        @DisplayName("Ciclo de vida - Paso 2: Clasificar solicitud → CLASIFICADA (sin prioridad)")
        void paso2_ClasificarSolicitud() throws Exception {
                ...
                mockMvc.perform(put("/api/solicitudes/" + solicitudId + "/clasificar")
                                ...
                                // Después de clasificar, NO debe haber prioridad asignada
                                .andExpect(jsonPath("$.datos.prioridad").doesNotExist())
        ...
        @Test
        @Order(3)
        @DisplayName("Ciclo de vida - Paso 2B: Priorizar solicitud")
        void paso2b_PriorizarSolicitud() throws Exception {
                PriorizacionRequestDTO request = PriorizacionRequestDTO.builder()
                                .prioridad(Prioridad.MEDIA)
                                .justificacion("Cancelación de asignatura con fecha límite próxima")
                                .build();
                ...
                mockMvc.perform(put("/api/solicitudes/" + solicitudId + "/priorizar")
                                ...
                                .andDo(print())
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.datos.prioridad").value("MEDIA"))
                                .andExpect(jsonPath("$.datos.justificacionPrioridad").isNotEmpty())
                                // RF-06: Historial debe tener 3 entradas (registro + clasificación + priorización)
        ...
        void consultarSolicitudesCerradas() throws Exception {
                mockMvc.perform(get("/api/solicitudes")
                                ...
                                .andExpect(jsonPath("$.datos.length()")
